...
交易员小镇后端服务，基于 FastAPI 构建，提供加密货币数据查询、模拟交易及用户资产管理功能。本项目集成智谱 GLM Agent 框架，并使用 SQLAlchemy 进行数据库管理。
...
5. **AI 交易员**: 集成智谱 GLM Agent，提供价值投资和技术面分析两种交易员
...
# 方式 B: 使用 pip
pip install -r requirements.txt  # 如果没有 requirements.txt，使用 pip install fastapi uvicorn sqlalchemy pydantic-settings httpx zhipuai
```
...
#### 3. 获取 OHLC 数据
- **方法**: `GET`
- **路径**: `/api/coins/{coin_id}/ohlc`
- **参数**: 
    - `coin_id`: 币种 ID
    - `vs_currency`: 计价货币 (默认 `usd`)
    - `days`: 天数 (默认 1)

*更多接口（如用户注册、下单、Agent 分析）正在开发中，请关注 `app/api/routes/` 目录。*
...
│   │   ├── value_investor_agent.py  # 价值投资交易员
│   │   ├── technical_analyst_agent.py  # 技术面分析交易员
│   │   └── models.py      # 数据模型
│   ├── db/
